...
class Exports:
    @staticmethod
    def export_video(export_type, num_frames, file_name, save_location='BASE'):
        if export_type not in ['axial', 'coronal', 'sagittal']:
            export_type = 'axial'
        if num_frames > 60 or num_frames < 0:
            num_frames = 30
        try:
            images_npy = np.load(r'data/{}.npy'.format(export_type))
        except Exception:
        ...
        if save_location == 'BASE':
            video = cv.VideoWriter('{}/{}.mp4'.format(file_name), cv.VideoWriter_fourcc(*'MP4V'), num_frames,
                                   (256, 256))
        else:
            video = cv.VideoWriter('{}/{}.mp4'.format(save_location, file_name), cv.VideoWriter_fourcc(*'MP4V'),
                                   num_frames, (256, 256))
        for i in images:
            video.write(i)
        video.release()
